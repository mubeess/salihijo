...
useEffect(()=>{
    ...
    if (appProps.editValue.user.section=='SSS') {
       ...
       fetch(`https://samsa-salihijo.herokuapp.com/admin/get-all-classes/?section=${appProps.editValue.user.section}`)
  .then(res=>{
    res.json()
    .then(data=>{
     ...
     <Typography style={{marginLeft:'10px'}} variant="button" display="block" gutterBottom>
      ...
      <div className='personal'>
        ...
        <Button onClick={()=>{
          ...
          fetch(`https://samsa-salihijo.herokuapp.com/admin/edit-student/?id=${appProps.editValue.user._id}`,{
            method:'PUT',
            headers:{
